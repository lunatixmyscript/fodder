...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/lunatixmyscript/lunaip/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/lunatixmyscript/lunaip/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/lunatixmyscript/lunaip/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
# LINK REPOSITORY
MENUREPO="https://raw.githubusercontent.com/lunatixmyscript/lunatixvpn/main/"
...
wget -O /etc/haproxy/haproxy.cfg "${CONFIG}haproxy.cfg" >/dev/null 2>&1
...
wget -O /etc/pam.d/common-password "${CONFIG}password"
...
wget -q -O /usr/local/lunatic/udp-mini "${CONFIG}udp-mini"
...
wget ${CONFIG}openvpn &&  chmod +x openvpn && ./openvpn
...
chronyc sourcestats -v
chronyc tracking -v
wget ${CONFIG}bbr.sh &&  chmod +x bbr.sh && ./bbr.sh
...
wget -O /usr/bin/ws "${CONFIG}ws" >/dev/null 2>&1
wget -O /usr/bin/tun.conf "${CONFIG}tun.conf" >/dev/null 2>&1
wget -O /etc/systemd/system/ws.service "${CONFIG}ws.service" >/dev/null 2>&1
...
wget -q -O /luna/run "${CONFIG}kill-ssh"
